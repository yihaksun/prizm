...
function DataAssetDetail({ onClose, onTrain, favorite, onFavorite }: { onClose: () => void; onTrain: () => void; favorite: boolean; onFavorite: () => void }) {
  const [version, setVersion] = useState(() => typeof window === 'undefined' ? 'v13' : new URLSearchParams(window.location.search).get('version') ?? 'v13');
  const [detailTab, setDetailTab] = useState(() => {
    const value = typeof window === 'undefined' ? null : new URLSearchParams(window.location.search).get('tab');
    return value && ['overview', 'samples', 'versions', 'lineage', 'access'].includes(value) ? value : 'overview';
  });
  const [sdkOpen, setSdkOpen] = useState(false);
  const updateDetailUrl = (tab: string, nextVersion = version) => {
    setDetailTab(tab);
    const params = new URLSearchParams(window.location.search);
    params.set('asset', 'PRJ000212-D-0001');
    params.set('tab', tab);
  ...
  return <PortalDetailFrame className="code-detail-page data-detail-page">
      ...
      <Tabs value={detailTab} onValueChange={(value) => updateDetailUrl(value)} className="code-detail-tabs">
